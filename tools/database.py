...
def needsdatabase(f):
    ...
    def wrap(*args, **kwargs):
        local("python manage.py syncdb --settings=quest.settings")
        return f(*args, **kwargs)
    return wrap

def remote_syncdb():
    local("heroku run python manage.py syncdb --settings=quest.settings")
...
def local_migrate(app_name):
    ...
    if not os.path.exists(os.path.join(app_name, "migrations")):
        ...
        with settings(warn_only=True):
            r = local("python manage.py schemamigration apps.%s --auto --settings=quest.settings" % app_name)

            if r.return_code != 0:
                print "Scema migration return code != 0 -> nothing to migrate"
            else:
                local("python manage.py migrate apps.%s --settings=quest.settings" % (app_name))